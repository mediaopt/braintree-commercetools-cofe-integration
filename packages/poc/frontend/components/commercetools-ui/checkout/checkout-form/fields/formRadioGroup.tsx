...
import { pngLogoWrapper } from '../logos/pngLogoWrapper';
          ...
          {options.map(({ id, value, label, isDefault, component }, index) => (
            <div key={index} className="grid gap-2">
              <div key={index} className="flex items-center">
                ...
                <label
                  htmlFor={id}
                  className="my-auto ml-5 flex h-7 w-fit max-w-full items-center text-sm font-medium text-gray-700 dark:text-light-100"
                >
                  {sVGLogoWrapper[id] ?? pngLogoWrapper(id) ?? <span className="m-auto">{label}</span>}
                </label>
              </div>
              {activePaymentMethod === id && component}
            </div>
          ))}
        </div>
      </fieldset>
